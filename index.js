...
app.get("/", async (req, res) => {
    ...
    if (page > maxPagination) page = maxPagination; //If the specified page number is greater than the last page index, use to the last page instead
  
    const docChunk = await scheme.find().limit(itemsPerPage).skip((itemsPerPage - 1) * 20); 
    // Fetch the main documents under the limit, and skip the documents accroding to the
